...
async function makeChatRequest(promptInput, apiKey) {
...
async function getResult(languages, frameworks, apiKey) {
  ...
  return await makeChatRequest(prompt, apiKey)
    .then((res) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="text-center text-gray-500 dark:text-gray-400 max-w-3xl">
          This usually takes around 30 seconds. But may take up to 2 minutes.
        </p>
      </div>
    </div>
  );
...
export default function Home() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const userKnowledge = sessionStorage.getItem("userKnowledge");
        const apiKey = sessionStorage.getItem("apiKey");

        if (!userKnowledge || !apiKey) {
          throw new Error("Missing session storage values.");
        }

        const parsedKnowledge = JSON.parse(userKnowledge);
        const result = await getResult(
          parsedKnowledge.lang,
          parsedKnowledge.framework,
          apiKey
        );
  ...
  if (error) {
    return (
      <main className="max-w-7xl mx-auto px-8 sm:px-16 w-screen" id="mainContent">
